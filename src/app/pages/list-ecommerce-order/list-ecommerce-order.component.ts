import { Component, OnInit, OnDestroy } from '@angular/core';
import { BaseComponent } from '../../modules/shared/base/base.component';
import { RestService } from '../../modules/shared/services/rest.service';
import { ActivatedRoute, Router } from '@angular/router';
import { Location } from '@angular/common';
import { Title } from '@angular/platform-browser';
import { OrderInfo } from '../../modules/shared/Models';
import { SearchObject } from '../../modules/shared/services/Rest';
import { CommonModule } from '@angular/common';
import { FormsModule } from '@angular/forms';
import { HeaderComponent } from '../../modules/shared/components/header/header.component';
import { LoadingComponent } from '../../modules/shared/components/loading/loading.component';
import { PaginationComponent } from '../../modules/shared/components/pagination/pagination.component';

@Component({
	selector: 'app-list-ecommerce-order',
	standalone: true,
	imports: [CommonModule, FormsModule, HeaderComponent, LoadingComponent, PaginationComponent],
	templateUrl: './list-ecommerce-order.component.html',
	styleUrl: './list-ecommerce-order.component.css'
})
export class ListEcommerceOrderComponent extends BaseComponent implements OnInit, OnDestroy {

	public rest_ecommerce_order: RestService;
	public ecommerce_orders: OrderInfo[] = [];
	public search_object: SearchObject<OrderInfo>;
	public rest_ecommerce_order = this.rest.initRestSimple<OrderInfo>('Order');

	ngOnInit(): void
{
		this.path = 'list-ecommerce-order';
		this.titleService.setTitle('Listado de Ordenes de Ecommerce');
		this.subs.sink = this.route.queryParamMap.subscribe(queryParams => {
			this.search_object = this.rest_ecommerce_order.getSearch(queryParams, ['id', 'client_name', 'total', 'status', 'created']);
			this.search(this.search_object);
		});
	}

	override search(search_object: SearchObject<OrderInfo>): void {
		this.is_loading = true;
		this.sink = this.rest_ecommerce_order.search(search_object).subscribe
		({
			next: (response) => {
				this.ecommerce_orders = response.data;
				this.setPages(response.current_page, response.total_items);
				this.is_loading = false;
			},
			error: (error) => {
				this.showError(error);
				this.is_loading = false;
			}
		});
	}

	delete(id: number): void
	{
		this.confirmation.confirm
		({
			message: '¿Estás seguro de que quieres eliminar esta orden?',
			accept: () => {
				this.is_loading = true;
				this.subs.sink = this.rest_ecommerce_order.delete(id).subscribe({
					next: () => {
						this.showSuccess('Orden eliminada exitosamente');
						this.search(this.search_object);
					},
					error: (error) => {
						this.showError(error);
						this.is_loading = false;
					}
				});
			}
		});
	}
}
