...
export class ItemSearchComponent extends BaseComponent implements OnInit, OnDestroy
{
	@Input() value: number | null | undefined = null;
	@Output() valueChange = new EventEmitter<number | null>();

	@Input() placeholder: string = 'Buscar artículo...';
	@Input() nullLabel: string = 'Seleccionar artículo';

	rest_all_items: RestSimple<Item> = this.rest.initRestSimple('item', ['id', 'name', 'code', 'status']);
	all_items: Item[] = [];
	filtered_items: Item[] = [];
	...
	ngOnInit(): void
	{
		this.rest_all_items.search({ eq: { status: 'ACTIVE' }, limit: 999999, sort_order: ['name_ASC'] })
			.subscribe({
				next: (response) =>
				{
					this.all_items = response.data;
					this.use_select = this.all_items.length <= 50;
					if (this.use_select && this.value)
					{
						let found = this.all_items.find(i => i.id === this.value);
						if (found)
						{
	...
	onSearchFocus(): void
	{
		...
		{
			this.filtered_items = this.all_items.slice(0, 20);
			this.selected_index = -1;
			this.show_dropdown = true;
		}
	}
	...
	{
		if (!this.show_dropdown || this.filtered_items.length === 0)
		{
			return;
		}
		...
		if (event.key === 'ArrowDown')
		{
			...
			this.selected_index = Math.min(this.selected_index + 1, this.filtered_items.length - 1);
		}
		...
		{
			event.preventDefault();
			if (this.selected_index >= 0 && this.selected_index < this.filtered_items.length)
			{
				this.onSelectItem(this.filtered_items[this.selected_index]);
			}
		}
