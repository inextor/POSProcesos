...
import {Rest, RestResponse, RestService, SearchObject} from '@shared/services/rest.service';
...
export class ViewOrderComponent extends BaseComponent implements OnInit
{
	...
	{
		let xml_attachment_id = this.order_info.sat_factura?.xml_attachment_id;

		if (!xml_attachment_id)
		{
			this.showError('La factura no tiene XML relacionado');
			return;
		}
		...
		try
		{
			let xml_response = await fetch(this.rest.getFilePath(xml_attachment_id), { credentials: 'include' });
			...
			let xml_string = await xml_response.text();
			let factura = this.parseFacturaXmlForPdf(xml_string);
			let agent = await this.getFacturaAgent();
			let html = this.buildFacturaPdfHtml(factura, agent);
			let payload = {
				html,
				orientation: 'P',
			...
			let response = await firstValueFrom(this.rest.callPostApi(url, payload, { responseType: 'blob' }));
			let blob = response instanceof Blob ? response : new Blob([response], { type: 'application/pdf' });
			let blob_url = window.URL.createObjectURL(blob);
			let a = document.createElement('a');
			a.href = blob_url;
			a.download = payload.download_name;
	...
	{
		let agent_id = this.order_info.client?.created_by_user_id;
	...
	private parseFacturaXmlForPdf(xml_string:string):FacturaPdfData
	{
		...
		let dom = parser.parseFromString(xml_string, 'application/xml');
		...
		let comprobante = this.getFirstXmlElement(dom, 'Comprobante');
		let emisor = this.getFirstXmlElement(dom, 'Emisor');
		let receptor = this.getFirstXmlElement(dom, 'Receptor');
		let timbre = this.getFirstXmlElement(dom, 'TimbreFiscalDigital');
		let conceptos = this.getXmlElements(dom, 'Concepto').map((concepto)=>
		{
			return {
		...
		return {
			...
			total: this.getXmlNumber(comprobante, 'Total'),
			...
			uuid: this.getXmlAttribute(timbre, 'UUID'),
			fechaTimbrado: this.getXmlAttribute(timbre, 'FechaTimbrado'),
			noCertificadoSat: this.getXmlAttribute(timbre, 'NoCertificadoSAT'),
			emisorNombre: this.getXmlAttribute(emisor, 'Nombre'),
			emisorRfc: this.getXmlAttribute(emisor, 'Rfc'),
			emisorRegimen: this.getXmlAttribute(emisor, 'RegimenFiscal'),
			receptorNombre: this.getXmlAttribute(receptor, 'Nombre'),
			receptorRfc: this.getXmlAttribute(receptor, 'Rfc'),
	...
	private buildFacturaPdfHtml(factura:FacturaPdfData, agent:User | null):string
	{
		let logo_url = this.rest.getImagePath(this.store.image_id, this.rest.preferences.logo_image_id);
				...
				<table class="agent-table">
					...
					<tbody>
						<tr>
							...
							<td>${this.escapeHtml(agent.phone || '')}</td>
		...
		let conceptos_html = factura.conceptos.map((concepto, index)=>
		{
			return `
				<tr class="${index % 2 == 1 ? 'alt' : ''}">
					<td class="num">${this.escapeHtml(concepto.cantidad)}</td>
					<td>${this.escapeHtml(concepto.unidad)}</td>
					<td>
						${this.escapeHtml(concepto.claveProdServ)}
						${concepto.noIdentificacion ? '<br>'+this.escapeHtml(concepto.noIdentificacion) : ''}
					</td>
					<td>${this.escapeHtml(concepto.descripcion)}</td>
					<td class="money">${this.formatPdfCurrency(concepto.valorUnitario)}</td>
					<td class="money">${this.formatPdfCurrency(concepto.importe)}</td>
				</tr>`;
		}).join('');
		...
		return `
			...
			<head>
				...
				<style>
					...
					.brand, .invoice-meta { display: table-cell; vertical-align: top; }
					.brand-logo { display: inline-block; width: 110px; height: 80px; margin-right: 22px; vertical-align: top; }
					.brand-info { display: inline-block; vertical-align: top; max-width: 420px; }
					.brand-info h1 { font-size: 22px; margin: 0 0 8px; }
					.invoice-meta { text-align: right; width: 270px; }
					.invoice-meta h2 { font-size: 26px; margin: 0 0 8px; }
					.grid { display: table; width: 100%; margin: 8px 0 16px; }
					.grid > div { display: table-cell; width: 50%; vertical-align: top; }
					.label { font-weight: bold; }
					.section-title { background: #ececef; font-weight: bold; padding: 4px 6px; margin-top: 8px; }
					.box { padding: 6px; min-height: 68px; }
					.fiscal-row { display: table; width: 100%; padding: 3px 6px; }
					.fiscal-row > div { display: table-cell; width: 50%; }
					table { border-collapse: collapse; width: 100%; }
					.agent-table { margin-top: 10px; margin-bottom: 10px; }
					th { background: #ececef; text-align: left; font-weight: bold; padding: 5px 6px; }
					td { padding: 5px 6px; vertical-align: top; }
					.items th { text-align: center; }
					.items .alt td { background: #eaf2ff; }
					.num, .money { text-align: right; white-space: nowrap; }
					.totals { margin-top: 10px; margin-left: auto; width: 260px; }
					.totals td:first-child { font-weight: bold; text-align: right; }
				</style>
			</head>
			<body>
				<div class="invoice">
					<div class="top">
						<div class="brand">
							<img class="brand-logo" src="${this.escapeHtml(logo_url)}">
							<div class="brand-info">
								<h1>${this.escapeHtml(factura.emisorNombre)}</h1>
								<div><span class="label">RFC:</span> ${this.escapeHtml(factura.emisorRfc)}</div>
								<div>${this.escapeHtml(this.store.address || '')}</div>
								<div>${this.escapeHtml([this.store.city, this.store.state, this.store.zipcode].filter(Boolean).join(', '))}</div>
							</div>
						</div>
						<div class="invoice-meta">
							<h2>Factura ${this.escapeHtml((factura.serie + ' ' + factura.folio).trim())}</h2>
							<div>Emitida</div>
							<div><span class="label">Fecha de emision:</span> ${this.escapeHtml(this.formatPdfDate(factura.fecha))}</div>
							<div><span class="label">Almacen:</span> ${this.escapeHtml(this.store.name || '')}</div>
						</div>
					</div>

					<div class="grid">
						<div>
							<div><span class="label">Folio SAT:</span> ${this.escapeHtml(factura.uuid)}</div>
							<div><span class="label">Lugar de expedicion:</span> ${this.escapeHtml(factura.lugarExpedicion)}</div>
							<div><span class="label">Fecha de certificacion:</span> ${this.escapeHtml(this.formatPdfDate(factura.fechaTimbrado))}</div>
						</div>
						<div>
							<div><span class="label">Certificado emisor:</span> ${this.escapeHtml(factura.noCertificado)}</div>
							<div><span class="label">Certificado SAT:</span> ${this.escapeHtml(factura.noCertificadoSat)}</div>
							<div><span class="label">Tipo de Comprobante:</span> ${this.escapeHtml(this.getTipoComprobanteLabel(factura.tipoComprobante))}</div>
						</div>
					</div>
					...
					<div class="grid">
						<div>
							...
							<div class="box">
								<div>${this.escapeHtml(factura.receptorNombre)}</div>
								<div>${this.escapeHtml(factura.receptorRfc)}</div>
								<div>${this.escapeHtml(factura.receptorDomicilio)}</div>
								<div><span class="label">Regimen fiscal receptor:</span> ${this.escapeHtml(factura.receptorRegimen)}</div>
							</div>
						</div>
						<div>
							<div class="section-title">Entregado a:</div>
							<div class="box">
								<div>${this.escapeHtml(this.order_info.order.client_name || this.order_info.client?.name || '')}</div>
								<div>${this.escapeHtml(this.order_info.order.address || '')}</div>
								<div>${this.escapeHtml([this.order_info.order.suburb, this.order_info.order.city, this.order_info.order.state, this.order_info.order.sat_codigo_postal].filter(Boolean).join(', '))}</div>
							</div>
						</div>
					</div>

					<div class="section-title">Datos adicionales para armado de comprobante fiscal digital:</div>
					<div class="fiscal-row">
						<div><span class="label">Metodo de pago:</span> ${this.escapeHtml(factura.metodoPago)}</div>
						<div><span class="label">Uso del CFDI:</span> ${this.escapeHtml(factura.usoCfdi)}</div>
					</div>
					<div class="fiscal-row">
						<div><span class="label">Forma de pago:</span> ${this.escapeHtml(factura.formaPago)}</div>
						<div><span class="label">Condiciones de pago:</span> ${this.escapeHtml(factura.condicionesPago)}</div>
					</div>
					<div class="fiscal-row">
						<div><span class="label">Regimen fiscal emisor:</span> ${this.escapeHtml(factura.emisorRegimen)}</div>
						<div></div>
					</div>
					...
					<table class="items">
						...
						<tbody>${conceptos_html}</tbody>
					</table>

					<table class="totals">
						<tr><td>Subtotal</td><td class="money">${this.formatPdfCurrency(factura.subtotal)}</td></tr>
						<tr><td>Total</td><td class="money">${this.formatPdfCurrency(factura.total)}</td></tr>
					</table>
				</div>
			</body>
			</html>`;
	...
	{
		let value = parseFloat(this.getXmlAttribute(element, attribute_name));
	...
	private formatPdfCurrency(value:number):string
	{
		return new Intl.NumberFormat('es-MX', {
			style: 'currency',
			currency: this.order_info.order.currency_id || 'MXN'
		}).format(value);
	}

	private getFacturaPdfDownloadName(factura:FacturaPdfData):string
	{
		let folio = `${factura.serie || ''}${factura.folio || ''}`.trim() || this.order_info.order.id.toString();
