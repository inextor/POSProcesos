...
import { Category, Item, Production, Requisition, Serial, Shipping, Stock_Record, Store } from '../../modules/shared/RestModels';
...
import { EMPTY, filter, forkJoin, mergeMap, of, Subject, switchMap } from 'rxjs';
...
export class SaveShippingComponent extends BaseComponent
{
	...
	private requisitionSearch$ = new Subject<{fecha: string, to_store_id: number | null}>();

	ngOnInit()
	{
		this.subs.sink = this.requisitionSearch$.pipe(
			switchMap(({fecha, to_store_id}) =>
			{
				...
				return this.rest_item_stock.search({search_extra:{store_id: this.from_store_id}, limit:9999}).pipe(
					...
					{
						let ids:number[] = itemResponse.data.map((i) => i.item.id);
			...
			next: (response) =>
			{
				if( response.requisitions && this.to_store_id )
				{
					...
					let ri_by_item_id = new Map<number, RequisitionItemInfo>();
		...
		({
			next: (responses) =>
			{
				...
				if ( this.to_store_id)
				{
					this.requisitionSearch(this.fecha_requisitions, this.to_store_id);
				}
				this.is_loading = false;
			},
			error: (error) =>
	...
	requisitionSearch(fecha: string, to_store_id: number | null)
	{
		this.requisitionSearch$.next({fecha, to_store_id});
	}
	...
	onToStoreChange(store_id:number)
	{
		if ( store_id )
		{
			...
			this.requisitionSearch(this.fecha_requisitions, store_id);
		}
	}
	...
	updateValues()
	{
		...
		this.shipping_info.items.forEach((sii: Partial<ShippingItemInfo>) =>
		{
			...
			{
				this.qty_by_item_id[sii.shipping_item.item_id] = sii.shipping_item.qty ?? 0;
			}
		});
	}
	...
	saveShipping(_evt:Event)
	{
		...
		{
			let bad_item = this.shipping_info?.items.find((citem) =>
				citem.shipping_item?.qty != null && citem.shipping_item.qty > (citem.available ?? 0)
			);
			this.showError('La cantidad a enviar no puede ser mayor al stock o menor a 0 (' + bad_item?.item?.name + ')');
			return;
		}

		this.is_loading = true;

		this.subs.sink = this.confirmation.showConfirmAlert(this.shipping_info,'Confirmar envío', '¿Desea confirmar el envío?')
		...
		.subscribe((response) =>
		{
			let on_response = {
				next: (_response:any) =>
				{
					this.showSuccess(this.shipping_info.shipping?.id ? 'El envío se actualizo exitosamente' : 'El envío se guardo exitosamente');
					this.is_loading = false;
