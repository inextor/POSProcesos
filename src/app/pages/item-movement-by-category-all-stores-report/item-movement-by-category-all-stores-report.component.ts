...
@Component({
	selector: 'app-item-movement-by-category-all-stores-report',
	templateUrl: './item-movement-by-category-all-stores-report.component.html',
	styleUrl: './item-movement-by-category-all-stores-report.component.css',
	imports: [CommonModule, FormsModule, RouterLink],
})
export class ItemMovementByCategoryAllStoresReportComponent extends BaseComponent implements OnInit
{
	...
	{
		return this.category_summaries.reduce((sum, row) => sum + (row.total_production_cost || 0), 0);
	}
	...
	ngOnInit(): void
	{
		...
		(
			...
			{
				this.setTitle('Reporte de Movimientos por Categoría - Todas las Sucursales');
				this.path = '/item-movement-by-category-all-stores-report';
		...
		({
			...
			next: (response) =>
			{
				...
				response.report.data.forEach(x =>
				{
					...
					let production_cost = (x.total_produced || 0) * (x.reference_price || 0);
					...
					g.total_production_cost += production_cost;
				});
	...
	onStoreChange(store_id: any)
	{
		...
		{
			this.router.navigate(['/item-movement-by-category-report'], {
				queryParams: {
		...
		this.category_summaries.sort((a, b) =>
		{
			...
			switch (column)
			{
				...
				case 'total_production_cost':
					aValue = a.total_production_cost || 0;
					bValue = b.total_production_cost || 0;
					break;
				default:
					return 0;
			}
