...
export class GetEmpty
{
	...
	static user_permission():User_Permission
	{
		return {
			...
			add_bills:0,
			add_commandas:0,
			add_consignment_delivered:0,
			add_consignment_received:0,
			add_consumptions:0,
			add_credit_limit:0,
			add_credit_sales:0,
			add_form:0,
			...
			discounts:0,
			edit_billing_data:0,
			edit_purchase_order_with_stock:0,
			factura_global:0,
			facturar_orden:0,
			fullfill_orders:0,
			global_add_stock:0,
			...
			view_asistance:0,
			view_commandas:0,
			view_consignment_delivered:0,
			view_consignment_received:0,
			view_global_stocktake:0,
			view_payroll:0,
			view_payrolls:0,
			view_reservations:0,
			view_responses:0,
	...
	static item_store(): any
	{
		return {
			...
			store_id: 0,
			pos_preference: 'SHOW',
			is_available_online: null,
			updated_by_user_id: 0,
			updated: new Date(),
		};
	}

	static category_store(): any
	{
		return {
			id: 0,
			created_by_user_id: 0,
