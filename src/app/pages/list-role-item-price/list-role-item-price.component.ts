...
{
	role_item_price: Production_Role_Price;
	item: Item;
	...
	ngOnInit(): void
	{
		...
		(
			mergeMap((param_map:any)=>
			{
				...
				return this.rest_role_item_price.search( this.role_item_price_search );
			})
			,mergeMap((response)=>
			{
				...
				search_item.csv.id = response.data.map((i:any)=>i.item_id);
				search_item.limit = 999999;

				return forkJoin
				({
					role_item_price_list: of( response.data ),
